...
const BookEvent = () => {
  ...
  const { loading, client, isClient } = useSelector((state) => state.client);
  ...
  return (
    <div className="container mx-auto">
      <div className="flex justify-between px-2 my-2">
        ...
        <span
          onClick={() => navigate(-1)}
          className="cursor-pointer text-white bg-blue-500 px-2 py-1 shadow"
        >
          <AiOutlineArrowLeft className="text-xl" />
        </span>
        <button onClick={logOutHandler} className="">
          {" "}
          <FiLogOut className="text-2xl text-white cursor-pointer" />
        </button>
      </div>
      <div className="w-full md:w-[70%] lg:w-[60%] mx-auto">
        {" "}
        <div className="flex flex-col md:flex-row bg-white border border-gray-200 py-8">
          <div className="flex-1 p-8 border-r border-gray-200 flex gap-2 flex-col">
            <div className="flex items-center gap-2">
              {" "}
              <h2 className="text-lg font-bold">{name}</h2>{" "}
              <span> with {bookedPerson}</span>
            </div>
            <div className="flex items-center gap-2">
