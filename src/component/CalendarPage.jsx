...
const CalendarPage = () => {
  ...
  useEffect(() => {
    if (user) {
      const eventData =
        user.event && user.event.find((item) => item._id === params.eventId);
      setEvent(eventData);
  ...
  function isTimeSlotAvailable(time, timeSlots, durationInMinutes) {
    ...
    for (let i = 0; i < timeSlots.length; i++) {
      const [slotStartTime, slotEndTime] = timeSlots[i]
        .split("-")
        .map((slot) => slot.trim());
      const slotStartTimeInMillis = moment(
        `01/01/2000 ${slotStartTime}`
      ).valueOf();
  ...
  for (let i = 0; i < timeSlots.length; i++) {
    const [startTime, endTime] = timeSlots[i]
      ...
      .map((slot) => slot.trim());
    timeIntervals.push(
      ...divideTimeRange(startTime, endTime, event && event.duration, timeSlots)
    );
  }
  ...
  useEffect(() => {
    ...
    const disabled = [0, 1, 2, 3, 4, 5, 6].filter(
      (day) => !weekdays.includes(Object.keys(weekdaysMap)[day])
    );
    ...
    if (events) {
      ...
      for (let i = 0; i < events.length; i++) {
        ...
        if (date1.isSame(date2)) {
          const [hours1, minutes1] = events[i].startTime
            .split(":")
            .map((part) => parseInt(part));
          const [hours2, minutes2] = events[i].endTime
            .split(":")
            .map((part) => parseInt(part));
          ...
          for (let j = 0; j < timeIntervals.length; j++) {
            ...
            if (
              (intervalStartTime.isSameOrAfter(eventStartTime) &&
                intervalStartTime.isBefore(eventEndTime)) ||
              (intervalEndTime.isAfter(eventStartTime) &&
                intervalEndTime.isSameOrBefore(eventEndTime))
            ) {
              index.push(j);
            }
          }
  ...
  const availableSlots = timeIntervals.filter((item, i) => {
  ...
  return (
    <div className="">
      <div className="flex justify-between px-2 my-2">
        ...
        <span
          onClick={() => navigate(-1)}
          className="cursor-pointer text-white bg-[#bad900] px-2 py-1 shadow"
        >
        ...
        <>
          <div className="flex flex-col md:flex-row bg-white container mx-auto border border-gray-200">
            ...
            <div className="flex-1 flex flex-col gap-2 px-8 border-l border-gray-200 mt-4 overflow-y-auto h-[90vh]">
              <button
                onClick={nextPage}
                className="border bg-blue-500 w-16  text-white px-2 py-1 text-center rounded shadow"
                disabled={time ? false : true}
              >
                Next
              </button>
              <div>{selectedDate && <p>{selectedDate}</p>}</div>
                ...
                {selectedDate &&
                  ...
                  availableSlots.map((item) => (
                    <div
                      ...
                      key={item}
                      className={`cursor-pointer border-2 text-center rounded shadow px-4 py-2 ${
                        time === item
